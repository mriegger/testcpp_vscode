...
namespace Matrix4x4 {
// From Lumberyard
float4x4 MakePerspectiveFovMatrixRH(float fovY, float aspectRatio,
  ...
  assert(fovY > FloatEpsilon &&
         aspectRatio >
             FloatEpsilon); // , "Both field of view in y direction and aspect
  ...
  out.SetRow(2, 0.f, 0.f, farDist / (nearDist - farDist),
...
float4x4 MakeOrthographicMatrixRH(float left, float right, float bottom,
  ...
  out.SetRow(0, 2.f / (right - left), 0.f, 0.f,
             -(right + left) / (right - left));
  out.SetRow(1, 0.f, 2.f / (top - bottom), 0.f,
             -(top + bottom) / (top - bottom));
  out.SetRow(2, 0.f, 0.f, 1 / (nearDist - farDist),
             nearDist / (nearDist - farDist));
